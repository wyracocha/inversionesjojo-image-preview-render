...
generate_image_render:
	...
	docker run \
		--rm \
		-e TZ=America/Lima \
		-v "$(PWD)/files/$(STL):/data/model.stl" \
		-v "$(PWD)/main.py:/data/main.py" \
		-v "$(PWD)/output:/data/output" \
		lscr.io/linuxserver/blender:latest blender -b -P /data/main.py -- /data/model.stl /data/output/$(basename $(STL))
generate_video_render:
	docker run \
		...
		-y \
		-framerate 24 \
		-i "/data/output/$(basename $(STL))/frame_%04d.png" \
		-c:v libx264 \
		-preset slow \
		-crf 18 \
		-pix_fmt yuv420p \
		/data/output/$(basename $(STL))/render_360.mp4
